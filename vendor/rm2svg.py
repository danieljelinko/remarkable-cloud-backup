...
#
# VENDORED + PATCHED for cloud-backup: maps firmware >=1.8 pen codes (12-21) onto
# the original 0-8 codes so ballpoint/fineliner/etc. strokes render instead of
# being dropped as "Unknown pen". Handles v3 and v5 .rm files only; v6 goes
# through rmc (see rmbackup.rmdoc_to_svg).
#
...
def rm2svg(input_file, output_name, coloured_annotations=False,
    ...
    for layer in range(nlayers):
        ...
        for stroke in range(nstrokes):
            ...
            if pen == 12: pen = 0     # Brush           (firmware >=1.8 pen codes)
            elif pen == 14: pen = 1   # Pencil
            elif pen in (15, 17): pen = 4  # Ballpoint / Fineliner
            elif pen == 16: pen = 3   # Marker
            elif pen == 18: pen = 5   # Highlighter
            elif pen == 13: pen = 7   # Mechanical pencil
            elif pen == 21: pen = 2   # Calligraphy
            if pen == 0 or pen == 1:
